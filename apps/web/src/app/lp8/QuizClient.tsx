...
function buildWonWhatsAppUrl(name: string, patologias: string[]) {
  ...
  const text = `Olá, me chamo ${name}.\n\nPatologias selecionadas:\n${list}\n\nAcabei de acertar 8 perguntas no quiz de mitos e verdades da Click Cannabis e quero resgatar minha consulta de acompanhamento gratuita!`;
  return whatsappUrl(text);
}

function buildLostWhatsAppUrl(name: string, patologias: string[]) {
  const list = patologias.map((p, i) => `${i + 1}. ${p}`).join("\n");
  const text = `Olá, me chamo ${name}.\n\nPatologias selecionadas:\n${list}\n\nJoguei o quiz de mitos e verdades da Click Cannabis e gostaria de saber mais sobre o tratamento com cannabis medicinal.`;
  return whatsappUrl(text);
}
  ...
  return (
    <>
      <section className="flex min-h-svh flex-col items-center justify-center px-6 py-8 text-center">
        ...
        >
          Acerte <span className="text-[#3a7a4f]">8 perguntas</span> e ganhe uma consulta de acompanhamento <span className="text-[#3a7a4f]">100% gratuita!</span>
        </h1>
        ...
        >
          Teste seus conhecimentos e acertando 8, você ganha uma <strong>consulta de acompanhamento gratuita ao iniciar seu tratamento na Click!</strong>
        </p>
...
function TopHud({
  ...
  const hasTimer = typeof timerKey === "number" && typeof onTimeout === "function";
  return (
    <header className="sticky top-0 z-10 w-full border-b border-gray-100 bg-white/85 backdrop-blur">
      <div className="mx-auto flex w-full max-w-2xl items-center justify-between gap-4 px-5 pt-3 pb-2">
        <div className="flex flex-1 items-center gap-1.5">
          {progress.map((filled, i) => (
            <div
              key={i}
              className={`h-1.5 flex-1 rounded-full transition-colors duration-300 ${filled ? "bg-[#3a7a4f]" : "bg-gray-200"}`}
            >
      ...
      <div
        className="mx-auto flex w-full max-w-2xl items-center justify-between gap-3 px-5 pb-2 text-[11px] font-medium tracking-wide text-gray-500 uppercase"
      >
        {hasTimer ? (
          <Timer key={timerKey} totalMs={TIMER_MS} onTimeout={onTimeout} />
        ) : (
          <span />
        )}
...
function Timer({
  ...
  return (
    <div className="min-w-[104px]" aria-label="Tempo restante">
      <div className="mb-1 flex items-center justify-center gap-1.5 rounded-full bg-gray-100 px-2.5 py-1">
        <span className="text-[9px] font-semibold tracking-widest text-gray-400 uppercase">Tempo</span>
        <span
          ref={textRef}
    ...
    <div className="flex min-h-svh w-full flex-col">
      ...
      <section className="flex flex-1 flex-col items-center justify-center px-5 py-5">
        <div
          ...
          className="q-scale-in mt-4 w-full max-w-2xl overflow-hidden rounded-3xl border border-gray-100 bg-white shadow-xl shadow-gray-900/5 opacity-0"
        >
          {question.image && (
            <div className="relative aspect-[16/9] w-full overflow-hidden bg-[#f0f7f1]">
              {question.image.src ? (
                <img
                ...
                <div className="flex h-full w-full items-center justify-center bg-[radial-gradient(circle_at_50%_42%,rgba(58,122,79,0.18),rgba(240,247,241,0.68)_45%,rgba(255,255,255,0.96)_100%)]">
                  <span
                    role="img"
                    aria-label={question.image.alt}
                    className="select-none text-[4.5rem] leading-none drop-shadow-sm sm:text-[6rem]"
                  >
    ...
    <div className="w-full">
      ...
      <section className="relative flex flex-col items-center overflow-hidden px-6 pb-12 pt-14 text-center">
        ...
        <p className="q-fade-up relative z-10 mt-4 max-w-[34ch] text-base leading-relaxed text-gray-600 opacity-0" style={{ animationDelay: "340ms" }}>
          Parabéns! Você acertou todas as 8 perguntas. Resgate agora sua consulta de acompanhamento gratuita.
        </p>
      ...
      {testimonials.length > 0 && (
        <TestimonialsSlider
          ...
          titleBold="iniciaram tratamento com a Click"
        />
      )}
      ...
      <section className="bg-white px-5 py-12 text-center">
        <div className="mx-auto max-w-xl">
          <h2 className="mb-3 text-2xl font-bold tracking-tight text-gray-900">Pronto para tirar suas dúvidas?</h2>
...
function WonLeadModal({ onClose, mode }: { onClose: () => void; mode: "won" | "lost" }) {
  ...
  function handleSubmit() {
    ...
    sendGTMEvent({
      ...
      category: "Lead",
      action: "Click",
        label: mode === "won" ? "Resgatar consulta - LP8" : "Iniciar tratamento - LP8",
...
function LostScreen({ correct, onRetry }: { correct: number; onRetry: () => void }) {
  ...
  return (
    <section className="flex min-h-svh flex-col items-center justify-center px-6 py-10 text-center">
      ...
      >
        Dessa vez não rolou a consulta gratuita — mas a Click ainda pode te ajudar a entender se esse tratamento faz sentido para você.
      </p>
