...
// Os únicos depoimentos do banco que falam de esporte e atividade física —
// quem chega do anúncio lê os dois ou três primeiros cards e mais nada, então
// eles abrem a faixa nesta ordem. A Juliana entra apesar da tag "Tea", que a
// regra acima excluiria: o que a qualifica aqui é a constância no treino.
const NOMES_ESPORTE = [
  "luiz carlos pizani",
  "Sandra CardosoBueno",
  "Juliana Lopes",
...
const ITEMS: TextTestimonial[] = (() => {
  const esporte = NOMES_ESPORTE.map((nome) =>
    TEXT_TESTIMONIALS.find((t) => t.name === nome),
  ).filter((t): t is TextTestimonial => t !== undefined);
  const jaNaFrente = new Set(esporte.map((t) => t.name));

  const aptos = TEXT_TESTIMONIALS.filter(
    (t) =>
      !jaNaFrente.has(t.name) &&
  ...
  return [...esporte, ...comDor, ...demais].slice(0, MAX_ITEMS);
})();
...
export default function TestimonialsWall() {
  ...
  return (
    <section id="lp15-testimonials-wall" className="overflow-hidden py-10 sm:py-16">
