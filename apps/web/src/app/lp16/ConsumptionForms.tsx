...
// Formas de consumo da lp5, com a pele escura da lp16.
    ...
    <section className="pt-12 pb-6 sm:pt-20">
      <div className="mx-auto w-full max-w-3xl px-5">
        <header className="mb-8 grid grid-cols-1 gap-3 sm:mb-12 sm:grid-cols-[1fr_auto] sm:items-start sm:gap-x-8 sm:gap-y-2">
          <span className="inline-flex w-fit items-center rounded-full px-3 py-1 text-xs font-medium text-[color:var(--green-ink)] sm:col-start-2 sm:row-start-1 sm:justify-self-end sm:text-sm" style={{ backgroundColor: "var(--accent-soft)" }}>
            Medicamentos
          </span>
        ...
        <div className="flex flex-col gap-1">
          {ITEMS.map((item, i) => {
            ...
            const headerId = `lp16-consumption-header-${i}`;
            const panelId = `lp16-consumption-panel-${i}`;
            return (
              <div
                key={item.num}
                className="rounded-2xl transition-colors duration-300 ease-out"
                style={{
                  backgroundColor: isActive ? "var(--green-soft)" : "transparent",
                }}
              >
                <h3 className="m-0">
                  <button
                    ...
                    className="flex w-full items-baseline gap-4 rounded-2xl px-5 py-4 text-left transition-transform duration-100 ease-out active:scale-[0.99] focus:outline-none focus-visible:ring-2 focus-visible:ring-[color:var(--green-500)] focus-visible:ring-offset-2 focus-visible:ring-offset-white sm:gap-5 sm:px-8 sm:py-6"
                  >
                    <span
                      className="shrink-0 tabular-nums text-sm transition-colors duration-300 sm:text-lg"
                      style={{ color: isActive ? "var(--green-ink)" : "var(--muted)" }}
                    >
                      {item.num}.
                    </span>
                    <span
                ...
                <div
                  ...
                  className="lp16-consumption-content grid overflow-hidden"
