...
export const metadata: Metadata = {
  title: "Quiz Click Cannabis — Ganhe uma Consulta Gratuita",
  description:
    "Acerte 10 perguntas sobre insônia e cannabis medicinal e ganhe uma consulta de acompanhamento gratuita no seu tratamento com a Click.",
};
...
async function getInsomniaTestimonials(): Promise<Testimonial[]> {
  try {
    ...
    const rows = (await sql`
      ...
      WHERE form_type = 'historia'
        ...
        AND id = ANY(${FEATURED_INSOMNIA_IDS})
    `) as TestimonialRow[];
    return rows.map((r) => {
...
export default async function LP6Page() {
  const testimonials = await getInsomniaTestimonials();
