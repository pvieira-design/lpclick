...
export default function LandingClient() {
    ...
    inactivityTimer.current = setTimeout(() => {
      ...
      if (!dialogRef.current?.open && currentSelected.size > 0) {
        ...
        sendGTMEvent({
          ...
          category: "Lead",
          action: "FormOpen",
          label: "Modal aberto (inatividade) - LP3",
  ...
  const openModal = useCallback(() => {
    resetInactivityTimer();
    setModalShowPicker(selected.size === 0);
  ...
  return (
    <>
      ...
      >
        ...
        {modalShowPicker ? (
          <div className="mt-4">
            <p className="mb-2 text-sm text-gray-500">Selecione suas patologias</p>
            <div
              className="grid grid-cols-1 gap-2 overflow-y-auto rounded-xl border border-gray-100 bg-gray-50/40 p-2"
              ...
              {PATOLOGIAS.map((p) => {
                ...
                return (
                  <button
                    ...
                    aria-checked={isSelected}
                    onClick={() => toggle(p)}
                    className="flex items-center gap-2 rounded-lg border-2 px-3 py-2.5 text-left text-sm font-medium transition-colors duration-150 ease-out select-none"
                    style={{
                      borderColor: isSelected ? "#3a7a4f" : "#e5e7eb",
                      backgroundColor: isSelected ? "#f0f7f1" : "white",
                      color: isSelected ? "#1a5c30" : "#374151",
                  ...
                  >
                    <span
                      className="flex size-4 shrink-0 items-center justify-center rounded-full border-2 transition-all duration-150 ease-out"
                      style={{
                        borderColor: isSelected ? "#3a7a4f" : "#d1d5db",
                        backgroundColor: isSelected ? "#3a7a4f" : "transparent",
                      }}
        ...
        <button
          ...
          style={{
            backgroundColor: selected.size > 0 ? "#1a5c30" : "#c5d4c9",
            cursor: selected.size > 0 ? "pointer" : "not-allowed",
          }}
        >
