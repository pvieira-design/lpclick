...
export default function TestimonialsSlider({
  ...
  return (
    <section className="relative overflow-hidden bg-white py-10 sm:py-16">
      <motion.div
        className="mx-auto mb-6 w-full max-w-xl px-5 text-center sm:mb-10"
        initial={{ opacity: 0, y: reduceMotion ? 0 : 20 }}
        whileInView={{ opacity: 1, y: 0 }}
        viewport={{ once: true, amount: 0.5 }}
        transition={{ duration: 0.6, ease: [0.23, 1, 0.32, 1] }}
      >
        <h2 className="font-display text-[1.5rem] leading-tight text-gray-900 sm:text-3xl">
          <span className="font-medium" style={{ color: "var(--muted)" }}>{titleLight}</span>{" "}
          <span className="font-semibold" style={{ color: "var(--green-700)" }}>{titleBold}</span>
        </h2>
      </motion.div>
      ...
      <motion.div
        ...
        transition={{ duration: 0.6, ease: [0.23, 1, 0.32, 1], delay: reduceMotion ? 0 : 0.1 }}
