...
export default function StickyCta() {
  ...
  useEffect(() => {
    const anchor = document.getElementById("lp14-sticky-anchor");
    if (!anchor) return;
  ...
  useEffect(() => {
    const ids = ["lp14-treatment-steps"];
    const targets = ids
  ...
  return (
    <>
      <style>{`
        @keyframes lp14StickyWave {
          from { box-shadow: 0 0 0 0 rgba(61,143,74,.55); }
          to { box-shadow: 0 0 0 18px rgba(61,143,74,0); }
        }
        .lp14-sticky-pulse { position: relative; isolation: isolate; }
        .lp14-sticky-pulse::after {
          content: "";
          position: absolute;
          inset: 0;
          border-radius: inherit;
          z-index: -1;
          animation: lp14StickyWave 1.6s cubic-bezier(.25,.8,.4,1) infinite;
          pointer-events: none;
        }
        @media (prefers-reduced-motion: reduce) {
          .lp14-sticky-pulse::after { animation: none; }
        }
      `}</style>
        ...
        <div className="mx-auto w-full max-w-xl">
          ...
          <motion.button
            ...
            className={`pointer-events-auto w-full rounded-2xl py-4 text-base font-bold text-white transition-all duration-150 ease-out hover:brightness-110 focus:outline-none focus-visible:ring-2 focus-visible:ring-[color:var(--green-500)] focus-visible:ring-offset-2 focus-visible:ring-offset-[#0B0D0B] sm:text-lg ${
              pulsing && visible ? "lp14-sticky-pulse" : ""
            }`}
